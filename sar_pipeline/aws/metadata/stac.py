...
class BurstH5toStacManager():
    ...
    def __init__(
        ...
        self.start_dt = isoparse(self.h5.search_value('identification/zeroDopplerStartTime'))
        self.end_dt = isoparse(self.h5.search_value('identification/zeroDopplerEndTime'))
        self.geometry_4326 = polygon_str_to_geojson(self.h5.search_value('boundingPolygon'))
    ...
    def add_properties_from_h5(self):
        ...
        self.item.properties['sat:orbit_state'] = self.h5.search_value('orbitPassDirection')
        self.item.properties['sat:absolute_orbit'] = self.h5.search_value('absoluteOrbitNumber')
        self.item.properties['sat:relative_orbit'] = 'trackNumber'
        self.item.properties['sat:orbit_cycle'] = '12'
        self.item.properties['sat:osv'] = self.h5.search_value('orbitFiles') # Link to a file containing the orbit state vectors.
        self.item.properties['sat:orbit_state_vectors'] = '' # map this from .h5

        # add the sar stac extension properties - see specs to add more
        self.item.properties['sar:instrument_mode'] = self.h5.search_value('acquisitionMode')
        self.item.properties['sar:frequency_band'] = self.h5.search_value('radarBand')
        self.item.properties['sar:center_frequency'] = self.h5.search_value('centerFrequency')
        self.item.properties['sar:polarizations'] = self.h5.search_value('listOfPolarizations')
        self.item.properties['sar:observation_direction'] = self.h5.search_value('lookDirection')
        ...
        self.item.properties['processing:level'] = self.h5.search_value('identification/productLevel')
        self.item.properties['processing:facility'] = 'Geoscience Australia'
        self.item.properties['processing:datetime'] = self.h5.search_value('identification/processingDateTime')
        self.item.properties['processing:version'] = self.h5.search_value('identification/productVersion')
        ...
        self.item.properties['card4l:noise_removal_applied'] = self.h5.search_value('noiseCorrectionApplied')
        self.item.properties['card4l:speckle_filtering'] = self.h5.search_value('filteringApplied')
        self.item.properties['card4l:pixel_coordinate_convention'] = 'Area'
        self.item.properties['card4l:measurement_type'] = self.h5.search_value('outputBackscatterNormalizationConvention')
        self.item.properties['card4l:measurement_convention'] = self.h5.search_value('outputBackscatterExpressionConvention')
        self.item.properties['card4l:conversion_eq'] = self.h5.search_value('outputBackscatterDecibelConversionEquation')
        self.item.properties['card4l:northern_geometric_accuracy'] = {
            'bias': self.h5.search_value('geometricAccuracy/bias/y'), # NOTE not north in 3031
            'std': self.h5.search_value('geometricAccuracy/stddev/y'), # NOTE not north in 3031
        }
        self.item.properties['card4l:eastern_geometric_accuracy'] = {
            'bias': self.h5.search_value('geometricAccuracy/bias/x'), # NOTE not north in 3031
            'std': self.h5.search_value('geometricAccuracy/stddev/x'), # NOTE not north in 3031
        }
        self.item.properties['card4l:gridding_convention'] = 'geocoded burst', # TODO is this a valid value to set?

        # add the storage stac extension properties
        self.item.properties['storage:type'] = 'aws-s3'
        self.item.properties['storage:platform'] = f'https://{self.s3_bucket}.s3.{self.s3_region}'
    ...
    def add_links_from_h5(self):
        ...
        self.item.add_link(
            pystac.Link(
                rel='derived_from',
                target=str(self.base_href / self.h5.search_value('sourceData/dataAccess')),
    ...
    def add_assets_from_folder(self, burst_folder: Path):
        ...
        INCLUDED_ASSET_FILETYPES = [x for x in REQUIRED_ASSET_FILETYPES if x not in IGNORE_ASSETS]
        ...
        for asset_filetype in INCLUDED_ASSET_FILETYPES:
            ...
            asset_filepaths = [x for x in burst_files if x.name == f'{self.id}{asset_filetype}']
            if len(asset_filepaths) == 0:
                raise FileNotFoundError(f'The required asset: "{asset_title}" is missing from burst folder: "{burst_folder}"')
            if len(asset_filepaths) > 1:
                raise ValueError(f'Expected 1 file for asset: "{asset_title}", {len(asset_filepaths)} found in burst folder: "{burst_folder}"')
